...
export function BillsView() {
  ...
  const handlePayBill = async () => {
    if (!selectedBillId || !selectedAccountId) return

    try {
      await updateBill(selectedBillId, { 
        status: "paid",
          ...
          <Card key={bill.id}>
            ...
            <CardContent>
              ...
              {bill.paymentAccountId && (
                <p className="text-sm text-muted-foreground mt-1">
                  Paid from: {accounts.find(acc => acc.id === bill.paymentAccountId)?.name}
                </p>
              )}
            </CardContent>
      ...
      <Dialog open={showPayBill} onOpenChange={setShowPayBill}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Select an account to pay the bill from</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowPayBill(false)}>
              Cancel
            </Button>
            <Button onClick={handlePayBill} disabled={!selectedAccountId}>
              Pay Bill
            </Button>
          </DialogFooter>
        </DialogContent>
