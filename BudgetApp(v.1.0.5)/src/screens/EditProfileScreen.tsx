...
export default function EditProfileScreen({ navigation }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If profile image has changed, save it locally
      if (profileImage && profileImage !== user?.photoURL) {
        console.log('Profile image has changed, saving locally...');
        try {
          // Save the image URI directly
          updateData.photoURL = profileImage;
        } catch (error) {
          console.error('Error saving profile image:', error);
          Alert.alert('Warning', 'Failed to save profile image. Profile name will still be updated.');
        }
      }
