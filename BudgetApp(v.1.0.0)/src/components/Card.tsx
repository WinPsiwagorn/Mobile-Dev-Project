...
export const Card: React.FC<CardProps> = ({ style, children, variant = "default", intensity = 20 }) => {
  const getCardStyle = () => {
    switch (variant) {
      case "glass":
  ...
  return (
    <View style={getCardStyle()}>
      {variant === "glass" && <BlurView intensity={intensity} style={StyleSheet.absoluteFill} />}
      {children}
    </View>
  )
}
